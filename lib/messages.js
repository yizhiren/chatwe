...
async function uploadFileChunk (basefilename, fileSize, chunkIdx, chunks, buffer, uploadmediarequest) {
  ...
  let webwxDataTicket = this.getWebwxDataTicket()
...
async function uploadFileTo (toUserName, filename) {
  ...
  let clientMediaId = util.getClientMsgId()
  ...
  for (let i = 0; i < chunks; i++) {
    ...
    resp = await this.uploadFileChunk(basename, fileSize, i, chunks, buffer.slice(0, realBytes), uploadmediarequest)
    let issucc = resp && resp.BaseResponse && resp.BaseResponse.Ret === 0
    if (!issucc) {
...
async function replyFileTo (userName, filename) {
  ...
  let resp = await this.uploadFileTo(userName, filename)
  let issucc = resp && resp.BaseResponse && resp.BaseResponse.Ret === 0
  if (!issucc) {
    return false
  }
...
module.exports.Register = function (core) {
  ...
  core.revoke = revoke
  core.getWebwxDataTicket = getWebwxDataTicket
}
