...
async function checkLogin () {
  ...
  let r = Math.floor(-localTime / 1579)
...
async function showMobileLogin () {
  ...
  return this.s(options)
    .then(function (resp) {
      ...
      logger.debug('showMobileLogin issucc :', issucc)
      if (!issucc) {
...
async function syncCheck () {
  ...
  return this.s(options)
    .then(function (resp) {
      ...
      let regx = /window.synccheck={retcode:"(\d+)",selector:"(\d+)"}/
      let group = regx.exec(resp)
      if (group && group[1] === '0') {
...
async function getMsg () {
  ...
  return this.s(options)
    .then(function (resp) {
      ...
      let issucc = resp && resp.BaseResponse && resp.BaseResponse.Ret === 0
      logger.debug('getMsg issucc :', issucc)
      if (issucc) {
        self.loginInfo['SyncKey'] = resp['SyncKey']
        let synckey = ''
  ...
  while (this.isLogined) {
    try {
      ...
      if (count < 0) {
        ...
        logger.debug('loopReceiving got:', messages)
        if (messages) {
