...
function registerTextHandler (handler) {
  return this.registerMessageHandler(MsgTypes.MSGTYPE_TEXT, handler)
}

function registerPictureHandler (handler) {
  return this.registerMessageHandler(MsgTypes.MSGTYPE_IMAGE, handler)
    .registerMessageHandler(MsgTypes.MSGTYPE_EMOTICON, handler)
}

function registerVoiceHandler (handler) {
  return this.registerMessageHandler(MsgTypes.MSGTYPE_VOICE, handler)
}

function registerVideoHandler (handler) {
  return this.registerMessageHandler(MsgTypes.MSGTYPE_VIDEO, handler)
    .registerMessageHandler(MsgTypes.MSGTYPE_MICROVIDEO, handler)
}

function registerSysHandler (handler) {
  return this.registerMessageHandler(MsgTypes.MSGTYPE_SYS, handler)
    .registerMessageHandler(MsgTypes.MSGTYPE_RECALLED, handler)
}

function registerFriendHandler (handler) {
  return this.registerMessageHandler(MsgTypes.MSGTYPE_VERIFYMSG, handler)
}

function registerCardHandler (handler) {
  return this.registerMessageHandler(MsgTypes.MSGTYPE_SHARECARD, handler)
}

function registerStatusHandler (handler) {
  return this.registerMessageHandler(MsgTypes.MSGTYPE_STATUSNOTIFY, handler)
}
...
function fillChatroomUser (fromType, fromUser, toType, toUser, msgToProcess) {
  ...
  if (toType === UserTypes.USER_TYPE_CHATROOM) {
    ...
    if (res) {
      ...
      let myDisplayName = myChatroomUser.DisplayName !== undefined ? myChatroomUser.DisplayName
        : this.getMyNickname()
...
async function produceMsg (msgList) {
  ...
  for (let key in msgList) {
    ...
    if (typeof (this.messageHandler[msg.MsgType]) === 'function') {
      await this.messageHandler[msg.MsgType].call(this, msgToProcess)
    } else {
