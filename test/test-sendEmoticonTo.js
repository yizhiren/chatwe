...
var s = http.createServer(function (req, res) {
  ...
  if (req.url.startsWith('/webwxsendemoticon')) {
    res.end(ret)
  }
})
...
test('wechat sendEmoticonTo 1', async function (t) {
  wechat.loginInfo['url'] = Config.BASE_URL
  wechat.loginInfo['User'] = {}
  let resp = await wechat.sendEmoticonTo('NAME', 'MEDIA')

  t.equal(0, resp.BaseResponse.Ret)
})

test('wechat sendEmoticonTo 2', async function (t) {
