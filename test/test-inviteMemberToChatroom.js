...
var s = http.createServer(function (req, res) {
  ...
  if (req.url.startsWith('/webwxupdatechatroom')) {
    ...
    req.on('end', function () {
      ...
      if (query.query.fun === 'invitemember' && body.InviteMemberList === '1,2' && body.ChatRoomName === roomName) {
        res.end(ret)
      }
    })
  }
...
test('wechat inviteMemberToChatroom 1', async function (t) {
  wechat.loginInfo['url'] = Config.BASE_URL
  wechat.loginInfo['User'] = {}

  let resp = await wechat.inviteMemberToChatroom({ UserName: roomName }, users)

  t.equal(0, resp.BaseResponse.Ret)
})

test('wechat inviteMemberToChatroom 2', async function (t) {
  wechat.loginInfo['url'] = Config.BASE_URL
  wechat.loginInfo['User'] = {}
