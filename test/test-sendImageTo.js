...
var s = http.createServer(function (req, res) {
  ...
  if (req.url.startsWith('/webwxsendmsgimg')) {
    res.end(ret)
  }
})
...
test('wechat sendImageTo 1', async function (t) {
  wechat.loginInfo['url'] = Config.BASE_URL
  wechat.loginInfo['User'] = {}
  let resp = await wechat.sendImageTo('NAME', 'MEDIA')

  t.equal(0, resp.BaseResponse.Ret)
})

test('wechat sendImageTo 2', async function (t) {
